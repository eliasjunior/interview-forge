...
export function buildSessionLaunchPrompt(session: Session): SessionLaunchPrompt {
  ...
  const prompt = session.interviewType === "code"
    ? [
        ...
        "Present the stored problem statement, ask the candidate to explain the approach and edge cases, then ask for working code or precise pseudocode.",
        "Give only light hints when needed and do not reveal the full solution.",
        "If get_session indicates that no executable challenge exists, call configure_code_challenge before asking for implementation. Generate starter code, visible sample tests, progressive hints, a private reference solution, and a private hidden-test harness.",
